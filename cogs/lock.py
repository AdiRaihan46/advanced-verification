...
# ------------------------------------------------------ #  
    ...
    async def lock (self, ctx, channel):
        ...
        if channel:
            ...
            embed = discord.Embed(title = f"#{channel.name} is now under lockdown!!", description = f"", color = 0x2fa737) # Green
            await ctx.channel.send(embed = embed)
        else:
            await ctx.channel.send("You dont have any rights to tell me anything!")
