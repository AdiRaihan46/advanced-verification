...
from cogs.utils.logMessage import sendLogMessage
...
# ------------------------------------------------------ #  
    ...
    async def on_member_join(self, member):
        ...
        # Check the user account creation date 
        if data["minAccountDate"] is False:
            ...
            if userAccountDate < data["minAccountDate"]:
                ...
                embed = discord.Embed(title = f"**YOU HAVE BEEN YEETED FROM {member.guild.name}**", description = f"Reason : Your account is too young! The server limit ({minAccountDate} hours).", color = 0xff0000)
        ...
        if data["captcha"] is True:
            ...
            font = ImageFont.truetype(font= "cogs/utils/arial.ttf", size= 60)
            ...
            captchaEmbed = await captchaChannel.send(f"**Please enter the captcha !**\nWelcome {member.mention}, you have 120 seconds to enter the captcha!", file= captchaFile)
            ...
            try:
                ...
                if msg.content == password:
                    ...
                    await msg.delete()
                    # Logs
                    embed = discord.Embed(title = f"**{member}'s iq is higher than 10.**", description = f"**__User informations :__**\n\n**Name :** {member}\n**Id :** {member.id}", color = 0x2fa737)
                    ...
                    await captchaChannel.send(embed = embed, delete_after = 5)
                    embed = discord.Embed(title = f"**YOU HAVE BEEN YEETED FROM {member.guild.name}**", description = f"Reason : You are stopid!\nServer link : <{link}>", color = 0xff0000)
                    await member.send(embed = embed)
                    await member.kick()
                    time.sleep(3)
                    await captchaEmbed.delete()
                    await msg.delete()
                    # Logs
                    embed = discord.Embed(title = f"**{member} has been yeeted.**", description = f"**Reason :** He is stopid.\n\n**__User informations :__**\n\n**Name :** {member}\n**Id :** {member.id}", color = 0xff0000)
                    embed.set_footer(text= f"at {memberTime}")
                    await sendLogMessage(self, event=member, channel=logChannel, embed=embed)
                ...
                await captchaChannel.send(embed = embed, delete_after = 5)
                try:
                    embed = discord.Embed(title = f"**YOU HAVE BEEN YEETED FROM {member.guild.name}**", description = f"Reason : You exceeded the captcha response time (120s).\nServer link : <{link}>", color = 0xff0000)
                    await member.send(embed = embed)
                    await member.kick()
                except Exception as error:
                ...
                await captchaEmbed.delete()
                # Logs
                embed = discord.Embed(title = f"**{member} has been yeeted.**", description = f"**Reason :** He exceeded the captcha response time (120s).\n\n**__User informations :__**\n\n**Name :** {member}\n**Id :** {member.id}", color = 0xff0000)
                embed.set_footer(text= f"at {memberTime}")
                await sendLogMessage(self, event=member, channel=logChannel, embed=embed)
